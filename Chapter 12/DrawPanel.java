...
public class DrawPanel extends JPanel {
    ...
    private MyShape createShape(int x1, int y1, int x2, int y2) {
        switch (shapeType) {
            case RECTANGLE:
                return new MyRectangle(x1, y1, x2, y2, currentColor, filledShape);
            case OVAL:
                return new MyOval(x1, y1, x2, y2, currentColor, filledShape);
            default:
                return new MyLine(x1, y1, x2, y2, currentColor, filledShape);
        }
    }

    // Single inner class handling all mouse + mouse-motion events
